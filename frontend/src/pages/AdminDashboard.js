...
const roleColor = { admin: "#10b981", analyst: "#818cf8", viewer: "#fbbf24" };
const roleBg    = { admin: "rgba(16,185,129,0.12)", analyst: "rgba(129,140,248,0.12)", viewer: "rgba(251,191,36,0.1)" };
const roleBorder= { admin: "rgba(16,185,129,0.3)", analyst: "rgba(129,140,248,0.3)", viewer: "rgba(251,191,36,0.25)" };
...
function StatCard({ label, value, sub, iconPath, color, trend }) {
  ...
  const c = colors[color] || colors.green;
  return (
    <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:"22px 24px", display:"flex", alignItems:"flex-start", gap:16, flex:1, minWidth:0, boxShadow:`0 4px 24px ${c.glow}`, transition:"transform 0.2s, box-shadow 0.2s", cursor:"default" }}
      onMouseEnter={e=>{ e.currentTarget.style.transform="translateY(-2px)"; e.currentTarget.style.boxShadow=`0 8px 32px ${c.glow}`; }}
      onMouseLeave={e=>{ e.currentTarget.style.transform=""; e.currentTarget.style.boxShadow=`0 4px 24px ${c.glow}`; }}>
      <div style={{ width:46, height:46, borderRadius:12, background:c.bg, border:`1px solid ${c.border}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
        <Icon d={iconPath} color={c.icon} size={20} />
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ fontSize:"0.72rem", color:"#64748b", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:4 }}>{label}</div>
...
function TxRow({ tx, onEdit, onDelete, userRole }) {
  ...
  return (
    <tr style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
      <td style={{ padding:"14px 16px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:isIncome?"rgba(16,185,129,0.12)":"rgba(248,113,113,0.1)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <Icon d={isIncome?icons.arrow_up:icons.arrow_down} color={isIncome?"#10b981":"#f87171"} size={16}/>
          </div>
          <div>
            <div style={{ fontWeight:600, color:"#e2e8f0", fontSize:"0.9rem" }}>{tx.title}</div>
            <div style={{ fontSize:"0.72rem", color:"#475569", marginTop:2 }}>{tx.category}</div>
      ...
      <td style={{ padding:"14px 16px", textAlign:"right" }}>
        <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.95rem", color:isIncome?"#10b981":"#f87171" }}>
          {isIncome?"+":`−`}₹{Number(tx.amount).toLocaleString("en-IN")}
        </span>
      </td>
      <td style={{ padding:"14px 16px" }}>
        <span style={{ background:isIncome?"rgba(16,185,129,0.1)":"rgba(248,113,113,0.1)", color:isIncome?"#10b981":"#f87171", border:`1px solid ${isIncome?"rgba(16,185,129,0.25)":"rgba(248,113,113,0.2)"}`, borderRadius:20, padding:"3px 10px", fontSize:"0.72rem", fontWeight:600, textTransform:"capitalize" }}>{tx.type}</span>
      </td>
      <td style={{ padding:"14px 16px", color:"#64748b", fontSize:"0.82rem" }}>
        {tx.date ? new Date(tx.date).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "—"}
      </td>
...
const btnStyle = color => ({
  background:`rgba(${color==="#818cf8"?"129,140,248":"248,113,113"},0.1)`,
  border:`1px solid ${color}33`, borderRadius:8,
  padding:"6px 8px", cursor:"pointer", display:"flex", alignItems:"center", transition:"all 0.15s",
});

// ── Record Modal ───────────────────────────────────────────────────────
function RecordModal({ mode, record, onClose, onSave, users, usersLoading, usersError }) {
  const [form, setForm] = useState(record || { title:"", amount:"", type:"income", category:"", date:"", notes:"", userId: "" });
  const set = (k,v) => setForm(f=>({...f,[k]:v}));
  const inputSt = { width:"100%", background:"rgba(255,255,255,0.04)", border:"1.5px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"11px 14px", color:"#e2e8f0", fontFamily:"'DM Sans',sans-serif", fontSize:"0.9rem", outline:"none", marginBottom:14 };
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(4px)" }} onClick={onClose}>
      <div style={{ background:"#0f172a", border:"1px solid rgba(255,255,255,0.1)", borderRadius:20, padding:32, width:420, maxWidth:"92vw", boxShadow:"0 24px 80px rgba(0,0,0,0.6)" }} onClick={e=>e.stopPropagation()}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:"1.2rem", color:"#fff", marginBottom:24 }}>
          {mode==="edit"?"Edit Record":"New Record"}
        </div>
        ...
        <textarea style={{...inputSt,resize:"vertical",minHeight:72}} placeholder="Notes (optional)" value={form.notes} onChange={e=>set("notes",e.target.value)}/>
        <div style={{ display:"flex", gap:12, marginTop:4 }}>
          <button onClick={onClose} style={{ flex:1, padding:"12px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"#64748b", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:600 }}>Cancel</button>
          <button onClick={()=>onSave(form)} style={{ flex:1, padding:"12px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#10b981,#059669)", color:"white", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700, boxShadow:"0 4px 16px rgba(16,185,129,0.3)" }}>Save</button>
        </div>
      </div>
    </div>
  );
...
function UserModal({ u, onClose, onSave }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(4px)" }} onClick={onClose}>
      <div style={{ background:"#0f172a", border:"1px solid rgba(255,255,255,0.1)", borderRadius:20, padding:32, width:380, maxWidth:"92vw", boxShadow:"0 24px 80px rgba(0,0,0,0.6)" }} onClick={e=>e.stopPropagation()}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:"1.1rem", color:"#fff", marginBottom:6 }}>Edit User</div>
        <div style={{ fontSize:"0.82rem", color:"#64748b", marginBottom:24 }}>{u.name} · {u.email}</div>

        {/* Role */}
        <div style={{ fontSize:"0.72rem", fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:"#94a3b8", marginBottom:10 }}>Role</div>
        <div style={{ display:"flex", gap:8, marginBottom:20 }}>
          {["viewer","analyst","admin"].map(r=>(
            <button key={r} onClick={()=>setRole(r)} style={{ flex:1, padding:"9px 6px", borderRadius:10, border:`1.5px solid ${role===r?roleColor[r]:"rgba(255,255,255,0.08)"}`, background:role===r?roleBg[r]:"transparent", color:role===r?roleColor[r]:"#64748b", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.8rem", textTransform:"capitalize", transition:"all 0.15s" }}>
              {r}
            </button>
          ))}
        </div>
        ...
        {/* Status */}
        <div style={{ fontSize:"0.72rem", fontWeight:700, letterSpacing:".08em", textTransform:"uppercase", color:"#94a3b8", marginBottom:10 }}>Status</div>
        <div style={{ display:"flex", gap:8, marginBottom:28 }}>
          {[{v:true,l:"Active"},{v:false,l:"Inactive"}].map(({v,l})=>(
            <button key={l} onClick={()=>setIsActive(v)} style={{ flex:1, padding:"9px 6px", borderRadius:10, border:`1.5px solid ${isActive===v?(v?"rgba(16,185,129,0.5)":"rgba(248,113,113,0.4)"):"rgba(255,255,255,0.08)"}`, background:isActive===v?(v?"rgba(16,185,129,0.1)":"rgba(248,113,113,0.1)"):"transparent", color:isActive===v?(v?"#10b981":"#f87171"):"#64748b", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.8rem", transition:"all 0.15s" }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={onClose} style={{ flex:1, padding:"12px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"#64748b", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:600, fontSize:"0.88rem" }}>Cancel</button>
          <button onClick={()=>onSave(u._id,{role,isActive})} style={{ flex:1, padding:"12px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#10b981,#059669)", color:"white", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"0.88rem" }}>Save Changes</button>
        </div>
      </div>
    </div>
  );
...
function AdminDashboard() {
  ...
  //fetch users
//   useEffect(() => {
//   if (activeNav !== "users" || user.role !== "admin") return;
...
useEffect(() => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this record?")) return;
    try { await API.delete(`/records/${id}`); setRecords(r=>r.filter(x=>x._id!==id)); } catch {}
  };
  const handleSave = async (form) => {
    try {
      if (modal.mode === "edit") {
  ...
  // ── User handlers ────────────────────────────────────────────────────
  const handleUserSave = async (id, changes) => {
    try {
      await API.put(`/users/${id}`, changes);
      setUsers(us => us.map(u => u._id === id ? { ...u, ...changes } : u));
      setUserModal(null);
    } catch (err) {
  ...
  const handleToggleStatus = async (u) => {
    try {
      ...
      setUsers(us => us.map(x => x._id === u._id ? { ...x, isActive: !u.isActive } : x));
    } catch {}
  };
  ...
  const countActive = users.filter(u=>u.isActive).length;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .user-row{border-bottom:1px solid rgba(255,255,255,0.04);transition:background 0.15s;}
        .user-row:hover{background:rgba(255,255,255,0.02);}
        .nav-btn{display:flex;align-items:center;width:100%;border:none;cursor:pointer;border-radius:12px;transition:all 0.15s;}
        .nav-btn:hover{background:rgba(255,255,255,0.04)!important;color:#94a3b8!important;}
      `}</style>

      <div style={{ display:"flex", minHeight:"100vh", background:"#080d1a" }}>

        {/* ── SIDEBAR ── */}
        <aside style={{ width:sidebarOpen?230:70, background:"rgba(10,15,30,0.95)", borderRight:"1px solid rgba(255,255,255,0.05)", display:"flex", flexDirection:"column", transition:"width 0.25s ease", overflow:"hidden", position:"sticky", top:0, height:"100vh", flexShrink:0, backdropFilter:"blur(20px)" }}>
          <div style={{ padding:"24px 20px", borderBottom:"1px solid rgba(255,255,255,0.05)", display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:34, height:34, borderRadius:9, background:"linear-gradient(135deg,#10b981,#059669)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:"0 4px 16px rgba(16,185,129,0.4)" }}>
          ...
          <nav style={{ flex:1, padding:"16px 10px", display:"flex", flexDirection:"column", gap:4 }}>
            {navItems.map(item => {
              const active = activeNav === item.key;
              return (
                <button key={item.key} className="nav-btn" onClick={()=>setActiveNav(item.key)} style={{ gap:12, padding:sidebarOpen?"11px 14px":"11px 0", justifyContent:sidebarOpen?"flex-start":"center", background:active?"rgba(16,185,129,0.12)":"transparent", color:active?"#10b981":"#475569" }}>
                  <Icon d={item.icon} color={active?"#10b981":"#475569"} size={18}/>
                  {sidebarOpen && <span style={{ fontWeight:500, fontSize:"0.88rem", whiteSpace:"nowrap" }}>{item.label}</span>}
                  {active && sidebarOpen && <div style={{ marginLeft:"auto", width:6, height:6, borderRadius:"50%", background:"#10b981" }}/>}
                </button>
          ...
          <div style={{ padding:"16px 10px", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
            {sidebarOpen && (
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", background:"rgba(255,255,255,0.03)", borderRadius:12, marginBottom:8 }}>
                <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#10b981,#059669)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontFamily:"'Syne',sans-serif", fontWeight:700, color:"white", fontSize:"0.82rem" }}>
                  {user.name?.[0]?.toUpperCase()||"A"}
                </div>
                <div style={{ minWidth:0 }}>
                  <div style={{ fontSize:"0.82rem", fontWeight:600, color:"#e2e8f0", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{user.name||user.email}</div>
                  <div style={{ fontSize:"0.68rem", color:"#10b981", textTransform:"capitalize", fontWeight:600 }}>{user.role}</div>
                </div>
              </div>
            )}
            <button onClick={()=>{ localStorage.clear(); window.location.href="/"; }} style={{ display:"flex", alignItems:"center", gap:10, padding:sidebarOpen?"10px 14px":"10px 0", justifyContent:sidebarOpen?"flex-start":"center", width:"100%", border:"none", background:"transparent", color:"#475569", cursor:"pointer", borderRadius:10, transition:"all 0.15s" }}
              onMouseEnter={e=>e.currentTarget.style.color="#f87171"} onMouseLeave={e=>e.currentTarget.style.color="#475569"}>
              <Icon d={icons.logout} size={17} color="currentColor"/>
              {sidebarOpen && <span style={{ fontSize:"0.85rem", fontWeight:500 }}>Sign Out</span>}
            </button>
        ...
        <main style={{ flex:1, overflow:"auto", minWidth:0 }}>
          ...
          {/* Topbar */}
          <div style={{ position:"sticky", top:0, zIndex:50, background:"rgba(8,13,26,0.9)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.05)", padding:"0 28px", height:64, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ display:"flex", alignItems:"center", gap:16 }}>
              <button onClick={()=>setSidebarOpen(s=>!s)} style={{ background:"none", border:"none", cursor:"pointer", color:"#475569", padding:4, display:"flex" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
              </button>
          ...
          <div style={{ padding:"28px 28px 48px" }}>
            ...
            {activeNav === "records" && (
              <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:18, padding:24 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, marginBottom:20, flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
                    <div style={{ position:"relative" }}>
                      <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}><Icon d={icons.search} size={15} color="#475569"/></span>
                      <input placeholder="Search…" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"9px 12px 9px 36px", color:"#e2e8f0", fontFamily:"'DM Sans',sans-serif", fontSize:"0.85rem", outline:"none", width:200 }}/>
                    </div>
                    <select value={filterType} onChange={e=>setFilterType(e.target.value)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"9px 14px", color:"#e2e8f0", fontFamily:"'DM Sans',sans-serif", fontSize:"0.85rem", outline:"none" }}>
                ...
                <div style={{ overflowX:"auto" }}>
                  <table>
                    <thead>
                      <tr style={{ borderBottom:"1px solid rgba(255,255,255,0.07)" }}>
                        {["Transaction","Amount","Type","Date","Actions"].map(h=>(
                          <th key={h} style={{ padding:"10px 16px", textAlign:h==="Amount"?"right":"left", fontSize:"0.72rem", fontWeight:700, color:"#475569", textTransform:"uppercase", letterSpacing:"0.07em", whiteSpace:"nowrap" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
            ...
            {activeNav === "users" && (
              <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
                ...
                <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
                  {[
                    { label:"Total Users",  value:users.length,        color:"#818cf8", bg:"rgba(129,140,248,0.12)", br:"rgba(129,140,248,0.25)" },
                    { label:"Active",       value:countActive,         color:"#10b981", bg:"rgba(16,185,129,0.12)",  br:"rgba(16,185,129,0.25)"  },
                    { label:"Admins",       value:countByRole("admin"), color:"#10b981", bg:"rgba(16,185,129,0.12)", br:"rgba(16,185,129,0.25)"  },
                    { label:"Analysts",     value:countByRole("analyst"),color:"#818cf8",bg:"rgba(129,140,248,0.12)",br:"rgba(129,140,248,0.25)" },
                    { label:"Viewers",      value:countByRole("viewer"), color:"#fbbf24",bg:"rgba(251,191,36,0.1)",  br:"rgba(251,191,36,0.25)"  },
                  ].map(s=>(
                    <div key={s.label} style={{ flex:1, minWidth:120, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:14, padding:"18px 20px" }}>
                      <div style={{ fontSize:"0.7rem", color:"#64748b", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6 }}>{s.label}</div>
                      <div style={{ fontFamily:"'Syne',sans-serif", fontSize:"1.8rem", fontWeight:800, color:s.color }}>{usersLoading?"—":s.value}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:18, padding:24 }}>
                  ...
                  {/* Header */}
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, marginBottom:20, flexWrap:"wrap" }}>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#f1f5f9", fontSize:"1.05rem" }}>
                      User Management
                      {!usersLoading && <span style={{ marginLeft:10, fontFamily:"'DM Sans',sans-serif", fontWeight:400, fontSize:"0.78rem", color:"#475569" }}>{filteredUsers.length} of {users.length} users</span>}
                    </div>
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
                      {/* Search */}
                      <div style={{ position:"relative" }}>
                        <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}><Icon d={icons.search} size={15} color="#475569"/></span>
                        <input placeholder="Search users…" value={userSearch} onChange={e=>setUserSearch(e.target.value)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"9px 12px 9px 36px", color:"#e2e8f0", fontFamily:"'DM Sans',sans-serif", fontSize:"0.85rem", outline:"none", width:200 }}/>
                      </div>
                      {/* Role filter */}
                      <select value={filterRole} onChange={e=>setFilterRole(e.target.value)} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"9px 14px", color:"#e2e8f0", fontFamily:"'DM Sans',sans-serif", fontSize:"0.85rem", outline:"none" }}>
                  ...
                  {usersLoading && (
                    <div style={{ textAlign:"center", padding:"60px 0" }}>
                      <div style={{ width:32, height:32, border:"3px solid rgba(16,185,129,0.2)", borderTopColor:"#10b981", borderRadius:"50%", animation:"spin 0.8s linear infinite", margin:"0 auto 14px" }}/>
                      <div style={{ color:"#475569", fontSize:"0.85rem" }}>Loading users…</div>
                    </div>
                  )}

                  {/* Error */}
                  {!usersLoading && usersError && (
                    <div style={{ background:"rgba(248,113,113,0.08)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:12, padding:"20px 24px", textAlign:"center" }}>
                      <div style={{ color:"#f87171", fontSize:"0.88rem", marginBottom:12 }}>{usersError}</div>
                      <button onClick={()=>setActiveNav("dashboard")} style={{ background:"rgba(248,113,113,0.1)", border:"1px solid rgba(248,113,113,0.25)", color:"#f87171", borderRadius:8, padding:"7px 16px", cursor:"pointer", fontSize:"0.82rem", fontWeight:600 }}>Go Back</button>
                    </div>
                  )}
                  ...
                  {!usersLoading && !usersError && (
                    <div style={{ overflowX:"auto" }}>
                        ...
                        <thead>
                          <tr style={{ borderBottom:"1px solid rgba(255,255,255,0.07)" }}>
                            {["User","Email","Role","Status","Joined","Actions"].map(h=>(
                              <th key={h} style={{ padding:"10px 16px", textAlign:"left", fontSize:"0.7rem", fontWeight:700, color:"#475569", textTransform:"uppercase", letterSpacing:"0.07em", whiteSpace:"nowrap" }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr key={u._id} className="user-row">
                              ...
                              <td style={{ padding:"14px 16px" }}>
                                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                                  <div style={{ width:36, height:36, borderRadius:"50%", background:`linear-gradient(135deg,${roleColor[u.role]||"#64748b"},${u.role==="admin"?"#059669":u.role==="analyst"?"#6366f1":"#d97706"})`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontFamily:"'Syne',sans-serif", fontWeight:700, color:"white", fontSize:"0.85rem" }}>
                                    {u.name?.[0]?.toUpperCase()||"U"}
                                  </div>
                                  <div style={{ fontWeight:600, color:"#e2e8f0", fontSize:"0.88rem" }}>{u.name}</div>
                                </div>
                              </td>

                              {/* Email */}
                              <td style={{ padding:"14px 16px", color:"#64748b", fontSize:"0.82rem" }}>{u.email}</td>

                              {/* Role badge */}
                              <td style={{ padding:"14px 16px" }}>
                                <span style={{ background:roleBg[u.role]||"rgba(100,116,139,0.1)", color:roleColor[u.role]||"#64748b", border:`1px solid ${roleBorder[u.role]||"rgba(100,116,139,0.2)"}`, borderRadius:20, padding:"3px 11px", fontSize:"0.72rem", fontWeight:700, textTransform:"capitalize" }}>
                                  {u.role}
                                </span>
                              </td>
                              ...
                              {/* Status toggle */}
                              <td style={{ padding:"14px 16px" }}>
                                <button onClick={()=>handleToggleStatus(u)} title="Click to toggle" style={{ background:u.isActive?"rgba(16,185,129,0.1)":"rgba(248,113,113,0.08)", color:u.isActive?"#10b981":"#f87171", border:`1px solid ${u.isActive?"rgba(16,185,129,0.3)":"rgba(248,113,113,0.2)"}`, borderRadius:20, padding:"3px 11px", fontSize:"0.72rem", fontWeight:700, cursor:"pointer", transition:"all 0.15s" }}>
                              ...
                              {/* Actions */}
                              <td style={{ padding:"14px 16px" }}>
                                <button onClick={()=>setUserModal(u)} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(129,140,248,0.1)", border:"1px solid rgba(129,140,248,0.25)", color:"#818cf8", borderRadius:8, padding:"6px 12px", cursor:"pointer", fontSize:"0.78rem", fontWeight:600, transition:"all 0.15s" }}>
                                  <Icon d={icons.edit} size={13} color="#818cf8"/> Edit
                                </button>
                              </td>
                            </tr>
                          ))}
                      ...
                      {filteredUsers.length===0 && !usersLoading && (
                        <div style={{ textAlign:"center", padding:"56px 0", color:"#475569", fontSize:"0.85rem" }}>
                          {users.length===0 ? "No users found. Make sure your /users API is connected." : "No users match your search."}
                        </div>
                      )}
                    </div>
                  )}
      ...
      {/* Modals */}
      {modal     && <RecordModal mode={modal.mode} record={modal.record} onClose={()=>setModal(null)} onSave={handleSave} users={users}/>}
      {userModal && <UserModal u={userModal} onClose={()=>setUserModal(null)} onSave={handleUserSave}/>}
    </>
  );
}
